...
- Reruns now reconcile the existing album to the desired state using the cheapest applicable operation, instead of always re-splitting. Specifically: a source file moved or renamed triggers a path-refresh only (no re-split, no retag); an album or artist folder name change triggers a folder move plus retag; a track title or performer change triggers a file rename plus retag; any per-track or album-level tag change triggers a retag in place. A full re-split now only occurs when the audio itself must change (audio stream changed (re-encode, different codec, different sample rate, channels, or time base), output format or codec mode changed, number of tracks changed, or a chapter boundary moved).
- The first run after upgrading from schema 3 reads the old manifest and, if nothing meaningful changed, rewrites it to schema 4 without re-splitting or re-tagging. Albums that predate the current cover-art or tag schema version are brought current (retag or cover recompose only) in that same first run.
