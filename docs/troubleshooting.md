...
The full output path for an album is: `<output>/<Artist>/<Festival Year (Stage)>/` for CrateDigger-tagged festival sets, `<output>/<Artist>/<Venue Year>/` (or `<Location Year>/` when only a location tag is present) for CrateDigger-tagged venue recordings without a festival tag, or `<output>/<Artist>/<filename-stem>/` for plain chaptered videos.
...
If you see a retag or re-split you were not expecting, check the debug log for the exact field that changed:
