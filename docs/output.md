...
The intro has no tracklist entry of its own, so it is credited to the set's artists: it carries the same `ARTIST`, `ARTISTS` and `MUSICBRAINZ_ARTISTID` tags as the album. Its filename stays `00 - Intro` regardless of how many artists that is.
...
- **`MUSICBRAINZ_ARTISTID`** is positionally aligned with `ARTISTS`, and is written only when *every* artist on the track resolves to a MusicBrainz ID. If even one is unknown the whole tag is omitted; `ARTISTS` still lists every artist by name. There is no way to mark a single missing ID in a positional list: an empty placeholder does not survive the round trip, because music servers that validate each value discard the empty one and then pair what is left by position, which silently gives each following artist the previous artist's identity. An absent tag costs an identifier that the artist almost always carries on other tracks; a shifted one asserts the wrong artist outright.
...
- **`MUSICBRAINZ_ALBUMARTISTID`** is positionally aligned with `ALBUMARTISTS` and follows the same all-or-nothing rule: written only when every album artist resolves, omitted otherwise.
...
1. **Environment variable:** if `CRATEDIGGER_DATA_DIR` is set and the directory exists, it is the sole source. CrateDigger honours the same variable, so exporting it points both tools at the same folder.
2. **Walk-up:** starting from the folder containing the input video, TrackSplit walks up to 10 parent directories looking for a `.cratedigger/` subfolder.
3. **CrateDigger's own data directory:** the folder where CrateDigger stores its curated data by default:
...
| `mbid_cache.json` | Fills missing MusicBrainz artist IDs. For Tier-1 sources (no CrateDigger tags) the `ALBUMARTIST` is looked up here to populate `MUSICBRAINZ_ALBUMARTISTID`. For Tier-2 sources, any gap in the MBID list (per-track `MUSICBRAINZ_ARTISTID` and album-level `MUSICBRAINZ_ALBUMARTISTID`) is backfilled from this cache. A name that is still unresolved after the backfill leaves the list incomplete, and an incomplete list is not written at all, so adding entries here is what makes those tags appear. |
| `artists/{slug}/dj-artwork.jpg` | DJ portrait used to compose the artist cover (`folder.jpg` / `artist.jpg`). Slugs are hyphenated (for example `martin-garrix`). Looked up by the CrateDigger canonical slug (from the `CRATEDIGGER_ALBUMARTIST_SLUGS` tag, index 0), used verbatim, hyphens and any `&` included. For files without that tag, the slug is derived from the artist name with `slugify`, which strips everything outside `a-z0-9` and produces a concatenated key (for example `martingarrix`); that key only matches cache folders created under the older concatenated naming, so files without the tag should be re-enriched with CrateDigger to get artwork matched under the current hyphenated folders. |
